...
// Minimal public holiday shape — PublicHolidayController wasn't provided,
// so keep this loose and adjust once its response shape is confirmed.
type PublicHoliday = {
  id: number;
  organization_id: number;
  name: string;
...
class AttendanceAPI {
  ...
  // NB: PublicHolidayController wasn't available for reference, so payload/
  // response shapes here are inferred from the route signatures only — verify
  // once that controller is shared.

  // GET /organizations/{org_id}/public-holidays?year=
  async getPublicHolidays(
    organizationId: number,
    year?: string | number,
  ): Promise<ApiResponse<PublicHoliday[]>> {
    try {
      const queryParams = this.buildQueryParams({ year });
      const url = `${
        process.env.NEXT_PUBLIC_BACKEND_API_URL
      ...
      return this.handleResponse<PublicHoliday[]>(response);
    } catch (error) {
      return {
        success: false,
        error:
  ...
  // POST /organizations/{org_id}/public-holidays
  async createPublicHoliday(
    organizationId: number,
    payload: Partial<PublicHoliday>,
  ): Promise<ApiResponse<PublicHoliday>> {
    try {
      const url = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/organizations/${organizationId}/public-holidays`;
  ...
  // PUT /organizations/{org_id}/public-holidays/{id}
  async updatePublicHoliday(
    ...
    try {
      const url = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/organizations/${organizationId}/public-holidays/${holidayId}`;
  ...
  // DELETE /organizations/{org_id}/public-holidays/{id}
