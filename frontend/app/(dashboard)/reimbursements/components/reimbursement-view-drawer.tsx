...
interface ReimbursementViewDrawerProps {
  open: boolean;
...
export function ReimbursementViewDrawer({
  open,
  onOpenChange,
  reimbursement,
  onItemsChanged,
}: ReimbursementViewDrawerProps) {
  const { user } = useAuth();
  ...
  const startAddItem = () => {
    setEditingItemId(0); // sentinel for "new"
    setItemForm({
      ...
      >
        <DrawerHeader className="border-b">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <DrawerDescription>
                {reimbursement.employee_full_name ||
                  `${reimbursement.employee_first_name ?? ""} ${reimbursement.employee_surname ?? ""}`}
              </DrawerDescription>
            </div>
          </div>
        </DrawerHeader>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{reimbursement.currency}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{formatDate(reimbursement.expense_date)}</p>
              </div>
              <div>
          ...
          {!!reimbursement.is_disputed && (
            <div>
              <h3 className="font-semibold mb-2 text-amber-700">Dispute</h3>
              <p className="text-sm text-amber-700 bg-amber-50 p-3 rounded-md">
                {reimbursement.disputed_reason}
                <br />
                <span className="text-xs text-amber-600">
                  Disputed {formatDateTime(reimbursement.disputed_at)}
                </span>
              </p>
            </div>
          )}

          {reimbursement.policy_validation_errors &&
            reimbursement.policy_validation_errors.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2 text-amber-700">Validation Notes</h3>
                <ul className="text-sm text-amber-700 bg-amber-50 p-3 rounded-md list-disc list-inside space-y-1">
                  {reimbursement.policy_validation_errors.map((err, i) => (
                    <li key={i}>{err}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="space-y-2">
              {auditTrail.map((log) => (
                <div key={log.id} className="text-sm border-l-2 border-gray-200 pl-3 py-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {log.firstname ? `${log.firstname} ${log.surname ?? ""} · ` : ""}
