...
import { Filter, Search, Plus, Eye, ChevronDownIcon } from "lucide-react";
import {
  leaveAPI,
  EmployeeLeaveType,
  LeaveFilters,
...
import { LeaveActionDialog } from "@/app/(dashboard)/leaves/components/leave-action-dialog";
import { LeaveViewDrawer } from "@/app/(dashboard)/leaves/components/leave-view-drawer";
...
const EmployeeLeaveTable: React.FC = () => {
  const { user } = useAuth();
  const [leaves, setLeaves] = useState<EmployeeLeaveType[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Drawer state
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [viewLeave, setViewLeave] = useState<EmployeeLeaveType | null>(null);
  ...
  }, [fetchLeaves, user?.organization_id, user?.employee?.id]);
  ...
  const handleViewClick = (leave: EmployeeLeaveType) => {
    setViewLeave(leave);
    setDrawerOpen(true);
  };
  ...
  const columns: ColumnDef<EmployeeLeaveType>[] = [
    {
      key: "leave_type",
      header: "Leave Type",
      cell: (leave) => (
    ...
    },
    {
      key: "approver",
      header: "Approver",
      cell: (leave) => leave.approver_full_name || "—",
    },
    {
      key: "actions",
      header: "Actions",
