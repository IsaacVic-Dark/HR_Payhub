...
class EmployeeController
{
    ...
    public function store($orgId)
    {
        try {
            ...
            if (!\App\Services\PermissionService::can($user['id'], 'employees.create')) {
                return responseJson(
                    success: false,
            ...
            DB::transaction(function () use ($data, $orgId, &$inserted, $default_password, $hasUser) {
                ...
                if ($hasUser) {
                    ...
                    // Add immediately after the new user row's id is resolved:
                    if (!empty($data['role'])) {
                        $roleRow = DB::raw(
                        ...
                        if (!empty($roleRow)) {
                            DB::raw(
                                ...
                                [':user_id' => $user_id, ':role_id' => $roleRow[0]->id]
                            );
                        }
                    }
                }
    ...
    public function update($orgId, $id)
    {
        try {
            ...
            $permScope = \App\Services\PermissionService::scopeOf($user['id'], 'employees.update');

            if ($permScope === 'own') {
                $allowedFields = ['phone', 'personal_email'];
            } elseif ($permScope === 'team') {
                $allowedFields = ['phone', 'job_title_id', 'department_id'];
            } elseif (\App\Services\PermissionService::can($user['id'], 'employees.update_payroll_fields') && $permScope !== 'all') {
                $allowedFields = ['base_salary', 'bank_account_number', 'tax_id'];
            } elseif (\App\Services\PermissionService::can($user['id'], 'employees.update_financial_fields') && $permScope !== 'all') {
                $allowedFields = ['bank_account_number', 'tax_id'];
            } elseif ($permScope === null) {
                return responseJson(success: false, message: 'You do not have permission to update employees', code: 403);
            }
            // $permScope === 'all' (admin/hr_manager): $allowedFields stays unset -> no filtering, matching original.
            ...
            if (isset($userData['user_type'])) {
                DB::raw("DELETE FROM model_has_roles WHERE user_id = :user_id", [':user_id' => $employee->user_id]);
                $newRole = DB::raw(
                    "SELECT id FROM roles WHERE organization_id = :org_id AND slug = :slug",
                    [':org_id' => $orgId, ':slug' => $userData['user_type']]
                );
                if (!empty($newRole)) {
                    DB::raw(
                        "INSERT INTO model_has_roles (user_id, role_id, created_at) VALUES (:user_id, :role_id, NOW())",
                        [':user_id' => $employee->user_id, ':role_id' => $newRole[0]->id]
                    );
                }
            }
    ...
    public function delete($orgId, $id)
    {
        try {
            ...
            $user = \App\Middleware\AuthMiddleware::getCurrentUser();
            if (!\App\Services\PermissionService::can($user['id'], 'employees.delete')) {
                return responseJson(
                    success: false,
    ...
    private function buildRoleBasedQuery($orgId, $roleFilters)
    {
        ...
        if (isset($roleFilters['full_access'])) {
    ...
    private function applyRoleBasedFilters($orgId)
    {
        ...
        $viewScope = \App\Services\PermissionService::scopeOf($user['id'], 'employees.view');
        if ($viewScope === null) {
        ...
        switch ($viewScope) {
            case 'all':
            case 'department':
                if (\App\Services\PermissionService::can($user['id'], 'employees.update')) {
                    $filters['full_access'] = true;
                } elseif (!\App\Services\PermissionService::canAny($user['id'], ['employees.update_payroll_fields', 'employees.update_financial_fields'])) {
        ...
        if (\App\Services\PermissionService::can($user['id'], 'employees.update_payroll_fields')) {
            $filters['payroll_access'] = true;
        }
        if (\App\Services\PermissionService::can($user['id'], 'employees.update_financial_fields')) {
